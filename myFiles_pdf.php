...
if($vr == 'no'){echo "<div class='eml_vrfy_note' style = 'color:white;background-color:#444;margin-top:10px;width:97%;border-radius:0 0 15px 0;padding-left:5px;font-family:verdana;font-size:13px;line-height:30px;'>Enter email verification code : <input type='text' id ='email_code' placeholder ='Enter your code' style='height:24px;width:95%;padding:0px;'/><button type='submit' id='email_code_submit' value='".$email."' style='background-color:#62B0FF;border:none;color:white;height:22px;border-radius:2px;margin-left:5px;'> Verify email address</button><div id='emerr'></div></div>";}
 ...
 ?>
          ...
          <div class="datagrid">
          <table><th colspan="2" style="padding:5px;">Document</th><th>Subject</th><th>Views</th>
    <?php
...
$query = "select * from electura_files_pdf";
...
$pagQuery="select * from electura_files_pdf WHERE user_id='$user_id' ORDER BY uploadDate DESC, uploadTime DESC LIMIT {$per_page} OFFSET {$pagination->offset()}";
...
if(!$pagResult) {
  ...
  while($row=mysql_fetch_array($pagResult)) {
    ...
    $likeQuery="select fileID from electura_comments_likes WHERE fileID='".$row['fileID']."'";
    $likeResult=mysql_query($likeQuery);
    $likeNum=mysql_num_rows($likeResult);

    // To get number of comments

    $commentQuery="select fileID from electura_comments_pdf WHERE fileID='".$row['fileID']."'";
...
//    $udate=$row['uDate'];
  ...
  //  $prof=$row['Professor'];
    ...
    "<small><b>File Size:</b> ".$filesize." | <b>Uploaded By:</b> ".$uploaded."<br />".$likeNum." <b>Like(s)</b> | ".$commentNum." <b>Comment(s)</b></small></td><td width='100px'>".$topicSubject."</td><td>".$views."</td></tr>";
    ...
    <?php
      if($pagination->total_pages()>1) {
        ...
        if($pagination->has_previous_page()) {
          echo "<li class='previous'><a href=\"myFiles_pdf.php?page=";
         ...
         for($i=1;$i<=$pagination->total_pages();$i++) {
          ...
          echo " <li><a href=\"myFiles_pdf.php?page={$i}\">{$i}</a> </li>";
        }

        }

        if($pagination->has_next_page()) {
          echo "<li class='next'><a href=\"myFiles_pdf.php?page=";
