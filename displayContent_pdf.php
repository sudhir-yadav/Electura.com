...
$pagQuery="select * from electura_files_pdf ORDER BY uploadDate DESC, uploadTime DESC LIMIT {$per_page} OFFSET {$pagination->offset()}";
    ...
    <?php
      if($pagination->total_pages()>1) {
        ...
        if($pagination->has_previous_page()) {
          echo "<li class='previous'><a href=\"displayContent_pdf.php?page=";
         ...
         for($i=1;$i<=$pagination->total_pages();$i++) {
          ...
          echo " <li><a href=\"displayContent_pdf.php?page={$i}\">{$i}</a> </li>";
        }

        }

        if($pagination->has_next_page()) {
          echo "<li class='next'><a href=\"displayContent_pdf.php?page=";
