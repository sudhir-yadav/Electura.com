...
$query = "select * from electura_files_video";
$result = mysql_query($query);
$totalCount = mysql_num_rows($result);

$pagination = new Pagination($page, $per_page, $totalCount);

$pagQuery="select * from electura_files_video ORDER BY topicDate DESC, topicTime DESC LIMIT {$per_page} OFFSET {$pagination->offset()}";
$pagResult=mysql_query($pagQuery);

    
  $query2="select * from electura_files_video";
...
//    $udate=$row['uDate'];
    ...
    $userQuery="select user_name from electura_user where user_id='$uploaded'";
    $userResult=mysql_query($userQuery);

    while($userRow=mysql_fetch_array($userResult)) {
      $uploaded=$userRow['user_name'];
    }
    $topicSubject=$row['topicSubject'];
  //  $prof=$row['Professor'];
    $thum=$row['thumbLink'];
    $totalViews=$row['topicViews'];
    //randomID, views, pages
    echo "<tr><td><a href=".$flink."><img src=".$thum." align='top'></img></a></td><td><a id='pad' href=".$flink.">".$fileName."</a><br/>".
    "<small>Uploaded By: ".$uploaded."</small></td><td width='100px'>".$topicSubject."</td><td>".$totalViews."</td></tr>";
    ...
    <?php
      if($pagination->total_pages()>1) {
        ...
        if($pagination->has_previous_page()) {
          echo "<li class='previous'><a href=\"displayContent_video.php?page=";
         ...
         for($i=1;$i<=$pagination->total_pages();$i++) {
          ...
          echo " <li><a href=\"displayContent_video.php?page={$i}\">{$i}</a> </li>";
        }

        }

        if($pagination->has_next_page()) {
          echo "<li class='next'><a href=\"displayContent_video.php?page=";
